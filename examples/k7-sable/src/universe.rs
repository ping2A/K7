//! Universe and SandApi for Sable (no_std, fixed grid). Port of PikuseruConsole/Sable universe.rs.
//! Tick loop lives in game.rs to avoid universe→species dependency.
...
impl Universe {
    ...
    /// Called at start of tick (before cell updates). Advances generation.
    pub fn prepare_tick(&mut self) {
        self.generation = self.generation.wrapping_add(1);
    }

    /// Clear the burn slot for (x,y) before running cell update.
    pub fn clear_burn(&mut self, x: i32, y: i32) {
        let idx = self.get_index(x, y);
        self.burns[idx] = EMPTY_WIND;
    }

    /// Called at end of tick. Copy burns→winds and advance generation.
    pub fn finish_tick(&mut self) {
        self.winds.copy_from_slice(&self.burns);
        self.generation = self.generation.wrapping_add(1);
    }

    /// Clear all cells to empty (like Sable reset).
            ...
            for dy in -floor..ceil {
                ...
                if c.species == Species::Empty || species == Species::Empty {
                    let ra = (60u8.saturating_add(size as u8))
                        .saturating_add((unsafe { frnd() } * 30.0) as u8)

# K7 Flocking / Boids (adapted from pikuseru-examples/demos/flocking/flock.pik)
# By laurent victorino; simplified: leader moves in circle (no input).
...
def update():
    ...
    for b in BOIDS:
        v1x, v1y = 0, 0  # toward center of mass
        cx, cy, n = 0, 0, 0
        for o in BOIDS:
            if o is not b:
                cx += o["x"]
        ...
        if n > 0:
            ...
            v1x = (cx - b["x"]) * 0.01
            v1y = (cy - b["y"]) * 0.01
        v2x, v2y = 0, 0  # separation
        for o in BOIDS:
            if o is not b and dist_vec(b, o) < MIN_DIST:
                v2x -= (o["x"] - b["x"])
                v2y -= (o["y"] - b["y"])
        v3x, v3y = 0, 0  # match velocity
        for o in BOIDS:
            if o is not b:
                v3x += o["vx"]
                v3y += o["vy"]
        if n > 0:
            v3x, v3y = (v3x / n - b["vx"]) * 0.125, (v3y / n - b["vy"]) * 0.125
